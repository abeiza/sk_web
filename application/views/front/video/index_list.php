			...
			$(function(){
				...
				$("#filter-date").change(
					function(){
						...
						$.ajax({
							url:"<?php echo base_url();?>index.php/berita/get_filter_index_video/",
							cache:false,
							data: {
							   filter : convdate,
							   kanal : kanal
							...
							success:function(result){
								...
								$.each(result, function(i, data){
									$('#grid-box-index .autor-list').append('<li>'+
										'<a href="<?php echo base_url();?>index.php/berita/video_detail/'+data.video_url+'" style="text-decoration:none;">'+
										'<div class="autor-box padding-full-10">'+
											'<div class="autor-content margin-left-0">'+
												'<div style="color:#FD8E23"><span class="padding-right-10">'+data.category_name+'</span><i class="fa fa-clock-o margin-right-5 padding-left-10" style="border-left:1px dotted #777;"></i>'+data.video_modify_date.substr(8,2)+" "+formatMonth(data.video_modify_date.substr(5,2).valueOf()-1)+" "+data.video_modify_date.substr(0,4)+'</div>'+
												'<h4><span>'+data.video_title+'</span></h4>'+
											'</div>'+
										'</div>'+
										'</a>'+
									'</li>');
