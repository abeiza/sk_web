<main class="mn-inner" style="background-color:#f1f1f1 !important;">
	<div class="row">
		<div class="col s12">
			<div class="page-title"><h5>Layout Single - Photo</h5>
				<div class="row">
					<div class="col s5">
						...
						<div style="width:100%;float:left;height:200px;">
							<div style="width:75%;height:100%;float:left;">
								...
								<div style="width:30%;float:left;">
									...
									<?php if($get_vertical_img == ''){?>
										<div id="lbl-vertical" style="border:1px dotted #f1f1f1;width:100%;height:100%;float:left;display:flex;margin:auto;"><span style="height:20px;margin:auto;font-size:12px;text-align:center"><strong style="color:#000">(Vertical Banner)</strong><br/>Click to Choose Banner<br/><strong id="get-vertical-side"></strong></span></div>
									<?php }else{?>
										<div id="lbl-vertical" style="background-position-x: center;background-position-y: center;background-size:contain;background-repeat:no-repeat;background-color:rgba(255,255,255,0.1);background-image:url('<?php echo base_url().'uploads/advertise/original/'.$get_vertical_img;?>');border:1px dotted #f1f1f1;width:100%;height:100%;float:left;display:flex;margin:auto;">
											<div style="width:100%;height:100%;background-color:rgba(0,0,0,0.5);float:left;display:flex;margin:auto;">	
												<span style="height:20px;margin:auto;font-size:12px;text-align:center;color:#fff;"><strong style="color:#fff">(Vertical Banner)</strong><br/>Click to Choose Banner<br/><strong id="get-vertical-side"></strong></span>
											</div>
										</div>

									<?php }?>
										<input type="hidden" id="vertical-banner"/>
									</div>
								</div>
								<div style="width:100%;float:left;height:80px;">
...
<div style="width:100%;position:fixed;height:100%;top:0;left:0;background-color:rgba(255,255,255,0.1);z-index:9999;" id="bg-pop">
	...
	<div id="vertical-pop" style="width:30%;padding:30px;text-align:center;height:230px;background-color:#fff;margin:auto;box-shadow:0 2px 2px 0 rgba(0,0,0,.19), 0 3px 1px -2px rgba(0,0,0,.19), 0 1px 5px 0 rgba(0,0,0,.19);">
		<h5 style="width:100%;">Select Banner</h5>
		<?php echo form_open('back/advertisement/set_banner/'.$this->uri->segment(4).'/'.$this->uri->segment(5));?>
		<select for="kanal" id="select-top-pop" name="selected_id">
			<option value="" disabled selected>Layout Data</option>
			<?php 
				foreach($get_list_vertical->result() as $vertical){
			?>
				<option value="<?php echo $vertical->adv_id;?>" <?php echo $vertical->adv_id == $get_vertical?'selected':'';?>><?php echo $vertical->adv_title;?></option>
			<?php
				}
			?>
			<option value="clear">Clear</option>
		</select>
		<input type="hidden" name="position_id" value='PSB00012'/>
		<button type="submit" class="waves-effect waves-light btn teal" id="btn-top-pop">Selected</button>
		<?php echo form_close();?>
	</div>
