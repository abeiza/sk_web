	...
	class Menus extends CI_Controller{
		...
		function index(){
			...
			if($cek){
				...
				$list['get_post'] = $this->db->query("select post_id, post_title, post_shrt_desc, category_name, flag_name, post_posted_by, post_status 
				from sk_post, sk_category, sk_user_back, sk_flag_post 
				where sk_user_back.user_back_id = sk_post.post_posted_by and sk_post.flag_id = sk_flag_post.flag_id and sk_post.category_id = sk_category.category_id 
				...
				$this->load->view('back/post/bottom_kanal');
			}else{
				$this->load->view('back/global/top');
			...
			if($this->form_validation->run() == false){
				...
				else
				{
					...
					if(!$add_data){
						...
						Header('Location:'.base_url().'index.php/back/kanal_side/');
					}else{
